...
class Trainer():
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, label_proj, proj_mask, label_proj_mask, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(train_loader):
            ...
            if self.gpu:
                ...
                if self.pncnn or self.completion_single_decoder_mask_prediction:
                    label_proj_mask = label_proj_mask.cuda()
                    label_proj = label_proj.cuda()

            # compute output
            # if self.completion_single_decoder:
            ...
            #             edges = cv2.Canny((label_proj_mask[batch_idx_edge_map]*255).detach().cpu().numpy().astype(np.uint8), 100, 200)
            ...
            #                      label_proj.permute(0, 2, 3, 1)[mask.bool()])
            #     loss_m = l1
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, label_proj, proj_mask, label_proj_mask, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(val_loader):
                proj_labels = label_proj
                ...
                #     wce = criterion(log_out, proj_labels)
                #     loss = wce + jacc + l1
                # print("self.completion_single_decoder", self.completion_single_decoder)
                if self.completion_single_decoder_mask_prediction: # important
                    completed_scene, completed_mask = model(in_vol, proj_mask)
                    mask = (proj_mask - 1) * (-1)  # not on projection mask
                    l1 = self.l1(completed_scene,  label_proj * mask.unsqueeze(1) + in_vol)
