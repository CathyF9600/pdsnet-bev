...
from tasks.bev.modules.ioueval import *
from tasks.bev.modules.SalsaNext import *
from tasks.bev.modules.SalsaNextInterpolate import *
from tasks.bev.modules.SalsaNextAdf import *
from tasks.bev.modules.SalsaNextCompletion import *
from tasks.bev.modules.SalsaNextCompletionSingleDecoder import *
from tasks.bev.modules.SalsaNextCompletionSingleDecoderStageTwo import *
from tasks.bev.modules.SalsaNextCompletionStageTwo import *
from tasks.bev.modules.pncnn import *
from tasks.bev.modules.SalsaNextCompletionStageTwoFilteredWeights import *
from tasks.bev.modules.SalsaNextCompletionAttention import *
from tasks.bev.modules.SalsaNextCompletionSingleDecoderMaskPredictionHead import *
from tasks.bev.modules.SalsaNextCompletionSingleDecoderStageTwoPerceptionLoss import *
from tasks.bev.modules.SalsaNextCompletionSingleDecoderStageTwoEndToEnd import *
from tasks.bev.modules.SalsaNextCompletionSingleDecoderMaskPredictionHeadStageTwo import *

from tasks.bev.modules.Lovasz_Softmax import Lovasz_softmax, MaskedProbExpLoss
import tasks.bev.modules.adf as adf
...
class Trainer():
    ...
    def make_log_img(input_depth, input_mask, pred_depth, depth, mask, pred, gt, color_fn, unfold_mask):
        ...
        # out_img = np.concatenate([out_img, pred_color], axis=0) # pred_color should be 3d not 4d
        # # make label gt
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, label_proj, proj_mask, label_proj_mask, path_seq, path_name) in enumerate(train_loader):
                       # proj_labels_input      proj_labels_mask
            # in_vol: input RV projected point cloud. range xyz remission
            # label_proj: gt RV projected point cloud. range xyz remission
            # proj_mask : 点云点在pixel上有没有点
            # label_proj_mask: gt点云点在pixel上有没有点
            # if i > 2:
            #     break
            # measure data loading time
            proj_labels = label_proj
            self.data_time_t.update(time.time() - end)
            if self.gpu:
                in_vol = in_vol.cuda()
                proj_mask = proj_mask.cuda().float()
                if self.pncnn or self.completion_single_decoder_mask_prediction:
                    label_proj_mask = label_proj_mask.cuda().float()
                    label_proj = label_proj.cuda()

            if self.completion_single_decoder_mask_prediction: # Important
                completed_scene, completed_mask = model(in_vol, proj_mask)
                mask = (proj_mask - 1) * (-1)  # not on projection mask
                l1 = self.l1(completed_scene,  label_proj * mask.unsqueeze(1) + in_vol)
                if self.edge_loss:
                    l1 = self.l1(completed_scene,
                                 label_proj)
                    bce = self.bce(completed_mask, label_proj_mask.float())
                    edge_mask = None
                    for batch_idx_edge_map in range(label_proj_mask.shape[0]):
                        # cv2.imwrite('mask_visualization.png', (proj_labels_mask[0]*255).detach().cpu().numpy())
                        edges = cv2.Canny(
                            (label_proj_mask[batch_idx_edge_map] * 255).detach().cpu().numpy().astype(np.uint8),
                            100, 200)
                    ...
                    l1 = self.l1(completed_scene[mask.bool()],
                                 label_proj[mask.bool()])
                    bce = self.bce(completed_mask, label_proj_mask.float())
                loss_m = l1 + bce
            ...
            if self.completion_single_decoder_mask_prediction:
                ...
                # accuracy non-sem TODO
                with torch.no_grad():
                    # evaluator.reset()
                    # argmax = output.argmax(dim=1)
                    # evaluator.addBatch(proj_mask, label_proj_mask)
                    # accuracy = evaluator.getacc()
                    accuracy = ((label_proj_mask == 1) & (proj_mask == 1)).sum().item() / (label_proj_mask == 1).sum().item()
                    mse_error = ((completed_scene[mask.bool()] - label_proj[mask.bool()]) ** 2).sum() / completed_scene[mask.bool()].shape[0]
                    # jaccard, class_jaccard = evaluator.getIoU()

                # acc.update(accuracy.item(), in_vol.size(0))
                acc.update(accuracy, in_vol.size(0))
                mse.update(mse_error.item(), in_vol.size(0))
            ...
            #     array = tensor.cpu().squeeze(0).detach().numpy()  # Remove the first dimension and convert to numpy array
            ...
            # # Example usage:
            # # Assuming `tensor` is your tensor of shape (1, H, W)
            # save_grayscale_image(completed_scene[0,:,:], './bev/%s.png' % str(datetime.datetime.now()))

            if show_scans:
                # get the first scan in batch and project points
                input_depth_np = in_vol[0][0].cpu().numpy()
                input_mask_np = proj_mask[0].cpu().numpy()
                mask_np = label_proj_mask[0].cpu().numpy()
                depth_np = label_proj[0][0].cpu().numpy()
                completed_scene_np = completed_scene[0][0].cpu().numpy()
                pred_np = None # argmax[0].cpu().numpy()
                gt_np = None # proj_labels[0].cpu().numpy()
                ...
                mask_np = label_proj_mask[1].cpu().numpy()
                completed_scene_np = completed_scene[1][0].cpu().numpy()
                depth_np = label_proj[1][0].cpu().numpy()
                pred_np = None # argmax[1].cpu().numpy()
                gt_np = None # proj_labels[1].cpu().numpy()
            ...
            if i % self.ARCH["train"]["report_batch"] == 0:
                print('Lr: {lr:.3e} | '
                    ...
                    data_time=self.data_time_t, loss=losses, l_one=l1_l, l_bce=bce_l, acc=acc, mse=mse, iou=iou, lr=lr,
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, label_proj, proj_mask, label_proj_mask, path_seq, path_name) in enumerate(val_loader):
                proj_labels = label_proj
                ...
                if self.gpu:
                    label_proj = label_proj.cuda(non_blocking=True).long()
            ...
            print('Validation set:\n'
                    ...
                    'WCE avg {wces.avg:.4f}\n'
                    'Acc avg {acc.avg:.3f}\n'
                    'mse avg {mse.avg:.3f}'.format(batch_time=self.batch_time_e,
                                                    loss=losses,
                                                    jac=jaccs,
                                                    l1ss=l1s,
                                                    wces=wces,
        ...
        return acc.avg, mse.avg, losses.avg, rand_imgs, hetero_l.avg
